...
class APIConfig:
    ...
    def HOST(self) -> str:
        """API server host"""
        return get_config('server_addresses.api_host', '0.0.0.0', 'api.server')
    
    @property
    def PORT(self) -> int:
        """API server port"""
        return get_config('system_ports.backend', 8001, 'api.server')
    ...
    def CORS_ORIGINS(self) -> List[str]:
        """CORS allowed origins - dynamically determined with comprehensive IP detection"""
        import socket
        import subprocess
        
        # Get frontend port from config
        frontend_port = get_config('system_ports.frontend', 3001, 'api.cors')
        ...
        # Method 2: Use ip route command (most reliable)
        try:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for ip in ips:
                    if ip.strip() and ip != '127.0.0.1':
                        origins.extend([
                            f"http://{ip.strip()}:{frontend_port}",
                            f"http://{ip.strip()}:3000"
                        ])
        except Exception:
            pass
            
        # Method 4: Network interface scanning
        try:
            for interface in ['eth0', 'wlan0', 'en0', 'ens33', 'enp0s3']:
                try:
        ...
        # For development: add wildcard for local development
        if get_config('server.api.debug', True, 'api.cors'):
            origins.extend([
